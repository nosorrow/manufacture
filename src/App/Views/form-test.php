    ...
    <div class="col-md-12">
      <form method="post" action="<?php echo site_url('store'); ?>" enctype="multipart/form-data">
        ...
        <div class="form-group">
          ...
          <input type="text" name="email[][female]"
                 class="form-control <?php echo ($errors->has('email.0.female')) ? 'is-invalid' : $valid; ?>"
                 id="email" placeholder="Enter email" value="<?php echo old('email.0.female'); ?>">
          <small id="emailHelp" class="invalid-feedback">
              <?php echo($errors->first('email.0.female')); ?>
          </small>
        </div>
        <div class="form-group">
          <label for="password">Password</label>
          <input name="email[][female]" type="text" value="<?php echo old('email.1.female'); ?>"
                 class="form-control <?php echo ($errors->has('email.1.female')) ? 'is-invalid' : $valid; ?>"
                 id="password" placeholder="Password">
          <small id="emailHelp" class="invalid-feedback">
              <?php echo($errors->first('email.1.female')); ?>
          </small>
        </div>
